...
import { getRestaurantBySlug, getRestaurantMenu, slugify } from "@lib/data/food"
import { notFound } from "next/navigation"

type Props = {
  params: Promise<{ countryCode: string; type: string; restaurantSlug: string }>
}
...
export default async function RestaurantPage({ params }: Props) {
  const { countryCode, type, restaurantSlug } = await params
  const typeLabel = decodeURIComponent(type)

  const restaurant = await getRestaurantBySlug(restaurantSlug)
  if (!restaurant) return notFound()

  const menu = await getRestaurantMenu(restaurant.restaurantID)
